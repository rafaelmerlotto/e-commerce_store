...
export async function loginAdmin(email: string, password: string): Promise<AdminSchema | null> {
    await dbConnect()
...
/******* Crete admin using only for development ********/
export async function createAdmin(email: string, password: string, verified: boolean): Promise<AdminSchema | null> {
    await dbConnect()
